...
TEST_F(MainDriverTest, CleanRun) {
	...
	array<PlayerResult, 2> player_results;
	thread main_runner(
...
TEST_F(MainDriverTest, Cancellation) {
	...
	// unique_ptr<LoggerMock> v_logger(new LoggerMock());
	// EXPECT_CALL(*v_logger, LogInstructionCount(PlayerId::PLAYER1,
	// _)).Times(1); EXPECT_CALL(*v_logger,
	// LogInstructionCount(PlayerId::PLAYER2, _)).Times(1);
	// EXPECT_CALL(*v_logger, LogFinalGameParams()).Times(1);
	// EXPECT_CALL(*v_logger, WriteGame(_)).Times(1);
